...
int main( int argc, char **argv )
{
  ...
  //---------------
	//Ruiying osg::ref_ptr<osgGA::CameraManipulator> cameraManipulator = new osgGA::TrackballManipulator;
	const osg::BoundingSphere& bs = loadedModel->getBound();

	if (bs.valid()) {
		// Adjust view to object view
		keyswitchManipulator->setHomePosition(osg::Vec3(0, bs.radius()*1.5, 0), osg::Vec3(0, 0, 0), osg::Vec3(0, 0, 1));
	}
	...
	//viewer.getCamera()->setComputeNearFarMode(osg::CullSettings::DO_NOT_COMPUTE_NEAR_FAR);//Ruiying: have to compute for haptics
	viewer.setCameraManipulator(keyswitchManipulator);
	// Subtract at least one bit of the node mask to disable rendering for main camera //Ruiying: don't disable rendering for main camera for osghaptics
 ...
 // osg::ref_ptr<osg::Group> root = new osg::Group;
  ...
  oculusViewer->addChild(visual_root.get());
  ...
  try {
    ...
    oculusViewer->addChild(haptic_root.get());
    ...
    */
    osgHaptics::prepareHapticCamera(viewer.getCamera(), haptic_device.get(), oculusViewer.get());
